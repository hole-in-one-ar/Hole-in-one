 ...
 */
float* matrixToQuaternion(const cv::Mat &Mat, float *q)
	{
	// shortcuts to the matrix data
	const float* m0 = (float*)( Mat.ptr<float>(0) );
	const float* m1 = (float*)( Mat.ptr<float>(1) );
	const float* m2 = (float*)( Mat.ptr<float>(2) );
 ...
 */
void getInitialPose( float* pRot, float *pTrans, const  cv::Point2f* p2D, float fMarkerSize, float f )
	{
	// compute homography
	float hom[ 3 ][ 3 ];
	...
	// R = C^-1 H S^-1
	float fRotMat[ 3 ][ 3 ];
	cv::Mat rotMat = cv::Mat( 3, 3, CV_32F, fRotMat[ 0 ] );
	...
	float fXLen = static_cast<float>(cv::norm(ColX));
	float fYLen = static_cast<float>(cv::norm(ColY));
...
//	cvScale( &ColY, &ColY, 1.0f / fYLen );
	...
	ColZ = ColX.cross(ColY);
//	cvCrossProduct( &ColX, &ColY, &ColZ );
	
	// normalize cross product	
	float fZLen = static_cast<float>( cv::norm( ColZ ) );
	ColZ *= 1.0f / fZLen;
//	cvScale( &ColZ, &ColZ, 1.0f / fZLen );
	
	// recompute y vector from x and z
	ColY = ColX.cross(ColZ);
	//cvCrossProduct( &ColX, &ColZ, &ColY );
	ColY *= -1.0f;
//	cvScale( &ColY, &ColY, -1.0 );
	ColX.copyTo(rotMat.col(0));
	ColY.copyTo(rotMat.col(1));
	ColZ.copyTo(rotMat.col(2));
 ...
 * @param f focal length
 */
void projectPoint( cv::Point2f &p2D, cv::Point3f p3D, const float* pRotation, const float* pTranslation, float f )
	{
	float point[ 3 ];
	float point3D[ 3 ];
	point3D[0] = p3D.x;
 ...
 * @param f focal length
 */
void computeJacobian( float* pResult, float* pParam, const cv::Point3f &p3D, float f )
	{
	// TODO: check for division by zero
 ...
 */
float computeReprojectionError( float* pError, const  cv::Point3f *p3D, const  cv::Point2f *p2D, int nPoints, 
 ...
 * @param f focal length
 */
void optimizePose( float* pRotation, float* pTranslation, int nPoints, const  cv::Point2f* p2D, const  cv::Point3f* p3D, float f )
	{
	using std::vector;
	...
	float paramDiff[ 7 ];
	vector<  cv::Point2f > estMeasurements( nPoints );
	...
	cv::Mat matJacobian(nPoints * 2, 7, CV_32F, &jacobian[ 0 ] );
	cv::Mat matJacobiSquare(7, 7, CV_32F, &jacobiSquare[ 0 ] );
	cv::Mat matMeasurementDiffPrev(nPoints * 2, 1, CV_32F, &measurementDiffPrev[ 0 ] );
	cv::Mat matMeasurementDiffNew(nPoints * 2, 1, CV_32F, &measurementDiffNew[ 0 ] );
	cv::Mat matParamDiff( 7, 1, CV_32F, &paramDiff[ 0 ] );
	cv::Mat matMDiff2(7, 1, CV_32F, &MDiff2[ 0 ] );
	...
	for ( int iIteration = 0; iIteration < nMaxIterations; iIteration++ )
		...
		matJacobiSquare = matJacobian.t() * matJacobian;
		//		matJacobiSquare = matJacobian * matJacobian.t();
//		cvMulTransposed( &matJacobian, &matJacobiSquare, 1 );
		matMDiff2 = matJacobian.t() * matMeasurementDiffPrev;
		...
		cv::solve(matJacobiSquare, matMDiff2, matParamDiff, cv::DECOMP_LU);
...
void estimateSquarePose( float* result, const cv::Point2f* p2D_, float markerSize ){
	cv::Point2f *p2D = new cv::Point2f[4];
 ...
 * @param markerSize side-length of marker. Origin is at marker center.
 */
void estimateSquarePose_( float* mat,  cv::Point2f* p2D, float markerSize )
	{
...
void calcHomography( float* pResult, const cv::Point2f *pQuad )
	...
	cv::Mat matA = cv::Mat(4, 3, CV_32F, fMatA[0]);

	// compute SVD
	// Idea: replace the whole thing with an analytical solution of the line at infinity
	float W[ 3 ];
	cv::Mat matW = cv::Mat( 3, 1, CV_32F, W );
	float U[ 3 ][ 3 ];
	cv::Mat matU = cv::Mat( 3, 3, CV_32F, U[ 0 ] );
	float V[ 3 ][ 3 ];
	cv::Mat matV = cv::Mat( 3, 3, CV_32F, V[ 0 ] );
	cv::SVDecomp(matA, matW, matU, matV, cv::SVD::MODIFY_A );
